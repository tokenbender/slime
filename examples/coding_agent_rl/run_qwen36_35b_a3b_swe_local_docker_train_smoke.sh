...
export MAX_CONTEXT_LEN="${MAX_CONTEXT_LEN:-512}"
export MAX_GEN_LEN="${MAX_GEN_LEN:-32}"
export ROLLOUT_MEM_UTILIZATION="${ROLLOUT_MEM_UTILIZATION:-0.20}"
...
export SLIME_AGENT_LOCAL_SCRIPT_COMMAND="${SLIME_AGENT_LOCAL_SCRIPT_COMMAND:-python -c \"from pathlib import Path; assert Path('slime/agent/harness/local_script.py').is_file(); print('OK')\"}"
export EXTRA_TRAIN_ARGS="${EXTRA_TRAIN_ARGS:---no-offload-train --sglang-max-total-tokens 4096 --sglang-max-running-requests 8 --sglang-server-concurrency 8}"
